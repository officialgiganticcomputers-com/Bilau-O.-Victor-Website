...
export default function BlueprintGenerator() {
  ...
  return (
    <section id="blueprint" className="py-32 bg-zinc-900 relative overflow-hidden border-t border-zinc-800 text-white">
      ...
      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-96 h-96 bg-amber-500/10 blur-[100px] rounded-full pointer-events-none"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16 fade-in-section">
          <div className="inline-flex items-center justify-center px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs font-bold uppercase tracking-widest mb-4 shadow-glow">Powered by Gemini AI ✨</div>
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-start fade-in-section">
          <div className="lg:col-span-5 bg-zinc-950/80 backdrop-blur-xl border border-zinc-800 p-8 rounded-3xl shadow-2xl">
            <form id="ai-blueprint-form" className="space-y-6" onSubmit={handleBpSubmit}>
              <div>
                ...
                <select id="bp-size" required className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500 transition-all font-light appearance-none" value={bpSize} onChange={e => setBpSize(e.target.value)}>
              ...
              <div>
                ...
                <select id="bp-focus" required className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500 transition-all font-light appearance-none" value={bpFocus} onChange={e => setBpFocus(e.target.value)}>
              ...
              <div>
                ...
                <textarea id="bp-challenge" required rows={3} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white placeholder-zinc-600 focus:outline-none focus:border-amber-500 transition-all resize-none font-light" placeholder="e.g. 'Our live stream audio drops frequently' or 'We need to train volunteers...'" value={bpChallenge} onChange={e => setBpChallenge(e.target.value)}></textarea>
              </div>
              <button type="submit" id="bp-submit" disabled={bpState === 'loading'} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-400 to-amber-600 hover:from-amber-300 hover:to-amber-500 text-zinc-950 font-bold py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(245,158,11,0.2)] disabled:opacity-70 disabled:cursor-not-allowed">
          ...
          <div className="lg:col-span-7 bg-zinc-950/50 backdrop-blur-xl border border-zinc-800 p-8 rounded-3xl min-h-[400px] flex flex-col relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-zinc-800 via-amber-500 to-zinc-800 opacity-20"></div>
            
            {bpState === 'initial' && (
              <div id="bp-initial-state" className="flex-1 flex flex-col items-center justify-center text-center opacity-60">
                <i className="ph-thin ph-cpu text-6xl text-amber-500 mb-4"></i>
            ...
            {bpState === 'loading' && (
              <div id="bp-loading-state" className="flex-1 flex flex-col items-center justify-center text-center">
                <div className="relative w-16 h-16 flex items-center justify-center mb-4">
                  ...
                  <i className="ph-fill ph-sparkle text-2xl text-amber-500 animate-pulse"></i>
                </div>
              ...
              <div id="bp-result-state" className="flex-1 flex flex-col text-zinc-300">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-zinc-800">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] uppercase tracking-widest text-amber-500 font-bold">Generated via Gemini AI ✨</p>
                  </div>
                </div>
